...
Podemos descargar **R** para windows directamente desde la siguiente dirección web
...
```{r echo=FALSE}
  knitr::include_graphics('./Figuras_tema_1/rproject_first.jpg')
```

En caso que trabajemos con otros sistemas operativos debemos seleccionarlo desde un **CRAN**, del inglés *Comprehensive R Archive Network*, que es el
...
```{r echo=FALSE, fig.align='center', out.height="100%", out.width="100%", fig.cap="Captura de pantalla de la página web donde se seleccionan el sistema operativo", results='asis'}

  knitr::include_graphics('./Figuras_tema_1/rproject_os.jpg')
...
Todas las cosas que manipula `R` se llaman objetos, en general, éstos se
construyen a partir de objetos más simples que pertenecen a alguna de
...
El objeto más simple es el vector, una de las muchas formas de crearlos
es a partir de elementos individuales.Para esto se utiliza la función
`c()`
...
renglón que comienza con el símbolo '[1]', indicando con ello que el
primer elemento del renglón corresponde al primer elemento del vector.

Dentro de un vector, sus elementos se pueden identificar mediante un
índice entero, que en el caso de este lenguaje empieza con el 1. Así,
por ejemplo:
...
También podemos usar los corchetes para seleccionar una secuencia de
elementos, o elementos particulares. Por ejemplo para un vector de 1000
números aleatorios, generados con la función `rnorm()`, que genera
números bajo una distribución normal(más adelante describiremos en
detalle está función):
...
Si queremos seleccionar sólo los números en las posiciones de la 5 a la
15:
...
Para seleccionar los elementos, 2, 3, y del 5 al 8, tanto en el vector
...
Los índices negativos, tienen el significado de excluir los elementos
señalados por ejemplo el complemento de los subconjuntos anteriores se
puede obtener así:
...
Como se mencionó en unidades anteriores un `data.frame` es un objeto con
apariencia de una tabla y una forma de crearlos es mediante la función
`data.frame()`. Para tener acceso a un elemento individual de un
`data.frame`, se utiliza el operador [,],el primer valor corresponde a
la fila y el segundo a la columna. Usaremos como ejemplo los datos
`trees` que vienen en `R`, para saber más acerca de estos datos tipee:
`?trees`
...
print("la primer columna")
datos[,1] # Selecciono la primer columna

print("el valor de la primer fila en la primer columna")
datos[1,1] # Selecciono el valor de la primer fila de la primer columna
```

Otro tipo de objeto es la clase `list` (lista), Esta es una clase de
datos que puede contener varios elementos, cada uno de los cuales puede
ser de una clase distinta.

```{r echo=FALSE, fig.align='center', fig.height=3, fig.width=3, out.width="70%"}
knitr::include_graphics("Figuras_tema_1/lista.jpg")
```

Por ejemplo, se puede concebir una lista para representar una familia:
la mamá,el papá, los años de casados, los hijos, y las edades de los
...
La lista familia contiene cinco elementos; los tres primeros son a su
vez de un sólo elemento: el nombre de la mamá, el nombre del papá, y los
años de casados. Los siguientes dos, son dos vectores de dos elementos
cada uno: los hijos y sus respectivas edades.

Para acceder a los elementos de una lista podemos utilizar el siguiente
operador `[[]]`,este no entrega ni una matriz, ni un `data.frame`, sino
un vector entero de un solo elemento, además este operador no admite ni
rangos, ni conjuntos de índices; esto es, el operador sólo admite un
índice entero o una cadena de caracteres que identifica el nombre de
...
La lista puede estar indexada por nombres. Para el ejemplo anterior,

```{r}
familia <- list('Madre' = "Maria", 'Padre' = "Juan", 'Años casados' = 10, 'Hijos' = c("Hugo", "Petra"), 'Edad hijos' = c(8,6))
familia
```

Notar que en general es indistinto el uso de comillas simples o dobles.

Recuerde que para crear nuevas variables y asignarlas a un objeto usamos
...
Las variables y objetos existentes en la memoria de `R` se pueden
listar, borrar, con los siguientes comandos:
...
Muchas veces resulta necesario conocer el tipo de las variables ya que
determinadas funciones operan de manera distinta (¡o fallan!)
dependiendo del tipo de la variable subyacente. Por lo cual se debe
comprobar que los datos con los que trabajamos son del tipo adecuado.

Con el siguiente código podemos ver con que tipo de variables trabajamos
...
En el caso de estructuras de datos complejas como listas o
`data.frame`,se puede conocer resumidamente el tipo de elementos que los
componen usando `str()`.

Este tipo de comprobaciones es importante hacerlo siempre después de la
lectura o la manipulación de datos para ver si se han procesado
correctamente.
...
|            |            |                            | datos?     |
+============+============+============================+============+
| `v e       | nu mérico, | una variable en el         | No         |
|    c to r` | al fa      | significado comúnmente     |            |
|            | bético, c  | asumido                    |            |
|            | omplejo o  |                            |            |
|            | lógico     |                            |            |
+------------+------------+----------------------------+------------+
| `f a       | factor n   | una variable categórica.   | No         |
|    c to r` | umérico o  |                            |            |
|            | a lf       |                            |            |
|            | abético    |                            |            |
+------------+------------+----------------------------+------------+
| a rr eg lo | nu mérico, | una tabla de dimensión k.  | No         |
| (          | a lf       |                            |            |
| `a         | abético c  |                            |            |
|    rr ay`) | omplejo o  |                            |            |
|            | lógico     |                            |            |
+------------+------------+----------------------------+------------+
| ma tr iz   | nu mérico, | un caso particular de un   | No         |
| (`ma       | al fa      | arreglo donde k = 2.       |            |
|    tr ix`) | bético, c  |                            |            |
|            | omplejo o  |                            |            |
|            | lógico     |                            |            |
+------------+------------+----------------------------+------------+
| `          | nu mérico, | un (marco o base de datos) | Si         |
| d  at a. f | al fa      | es una tabla compuesta de  |            |
|    r am e` | bético, c  | uno o más vectores y/o     |            |
|            | omplejo o  | factores de la misma       |            |
|            | lógico     | longitud pero que pueden   |            |
|            |            | ser de diferentes tipos    |            |
+------------+------------+----------------------------+------------+
| `t s`      | nu mérico, | una serie temporal y como  | Si         |
|            | al fa      | tal contiene atributos     |            |
|            | bético, c  | adicionales tales como     |            |
|            | omplejo o  | frecuencia y fechas        |            |
|            | lógico     |                            |            |
+------------+------------+----------------------------+------------+
| l is ta    | nu mérico, | puede contener cualquier   | Si         |
| (`l        | a lf       | tipo de objeto incluyendo  |            |
|    i st )` | abético co | otras listas!              |            |
|            | mplejo,    |                            |            |
|            | lógico     |                            |            |
+------------+------------+----------------------------+------------+
...
|                | escaleras, los datos se representan como la      |
|                | parte superior de las líneas verticales, "S":    |
|                | igual al anterior pero los datos se representan  |
|                | como la parte inferior de las líneas verticales. |
+----------------+--------------------------------------------------+
| \` x l i m = , | especifica los límites inferiores y superiores   |
|                | de los ejes; por ejemplo con `xlim=c(1, 10)` o   |
| y l i m = \`   | `xlim=range(x)`,`xlab=`, `ylab=` títulos en los  |
|                | ejes; deben ser variables de tipo carácter       |
+----------------+--------------------------------------------------+
| `m  a  i n =`  | título principal; debe ser de tipo carácter      |
+----------------+--------------------------------------------------+
...
| o           |                                                       |
+=============+=======================================================+
| `a d d = F  | si es TRUE superpone el gráfico en el ya existente    |
|  A   L S E` | (si existe)                                           |
+-------------+-------------------------------------------------------+
| `a x e s =  | si es FALSE no dibuja los ejes ni la caja del gráfico |
|  T   R U E` |                                                       |
+-------------+-------------------------------------------------------+
| `t y p e    | Especifica el tipo de gráfico; "p": puntos, "l":      |
|  =   " p "` | líneas, "b": puntos conectados por líneas, .o": igual |
|             | al anterior, pero las l´ıneas están sobre los puntos, |
|             | "h": líneas verticales, "s": escaleras, los datos se  |
|             | representan como la parte superior de las líneas      |
|             | verticales, "S": igual al anterior pero los datos se  |
|             | representan como la parte inferior de las líneas      |
|             | verticales.                                           |
+-------------+-------------------------------------------------------+
...
| x, y,   na     | especificado por side (ver axis() más abajo); line |
| mes)   mtex    | especifica la línea del área de graficado segments |
|   t( text, s   | (x0,y0, x1, y1)dibuja una línea desde el punto     |
|   id e=3 , li  | (x0,y0) hasta el punto (x1,y1)                     |
|    n e=0,...)` |                                                    |
+----------------+----------------------------------------------------+
| `ar  row   s   | igual al anterior pero con flechas desde (x0,y0)   |
|  (x0,    y0    | si code=2, al punto (x1,y1) si code=1, o en ambos  |
| ,   x 1, y1,   | si code=3; angle controla el ángulo desde la base  |
|   an gle = 30  | de la flecha hasta la punta de la misma            |
|    ,  code=2)` |                                                    |
+----------------+----------------------------------------------------+
...
|    e (lm.obj)` |                                                    |
+----------------+----------------------------------------------------+
| `rec           | dibuja un rectángulo donde las esquinas izquierda, |
|   t( x1,   y1  | derecha, superior e inferior están dadas por x1,   |
|    ,  x2, y2)` | x2, y1, y y2, respectivamente                      |
+----------------+----------------------------------------------------+
| `po lygo       | dibuja un polígono uniendo los puntos dados por x  |
|    n (x,   y)` | y y                                                |
+----------------+----------------------------------------------------+
| `lege          | agrega la leyenda en el punto (x,y) con símbolos   |
|   nd (x,    y  | dados por legend                                   |
|    ,  legend)` |                                                    |
+----------------+----------------------------------------------------+
| `title()`      | agrega un título y opcionalmente un sub-titulo     |
+----------------+----------------------------------------------------+
| `ax is( side   | agrega un eje en la parte inferior (side=1),       |
|    ,    vect)` | izquierda (2), superior (3), o derecha (4); vect   |
|                | (opcional) da la abscisa (u ordenada) donde se     |
|                | deben dibujar los marcadores('tick marks') del eje |
+----------------+----------------------------------------------------+
| `rug(x)`       | dibuja los datos x en el eje x como pequeñas       |
|                | líneas verticales                                  |
+----------------+----------------------------------------------------+
| `lo cator(     | con el ratón; también dibuja símbolos (type="p") o |
|   n,   t ype=  | líneas (type="l") con respecto a parámetros        |
|    " n", ...)` | gráficos opcionales (...); por defecto no se       |
|                | dibuja nada (type="n")                             |
+----------------+----------------------------------------------------+
| `id enti       | Similar a locator() con la diferencia que imprime  |
|    f y(x,...)` | en la gráfica el valor de x (u opcionalmente de    |
|                | una leyenda especificada en la opción labels=) más |
|                | cercano al punto donde se hizo click. Útil para    |
|                | identificar puntos en la gráfica que están         |
